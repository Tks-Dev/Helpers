...
namespace Crypto
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var it = 0;
            foreach (var c in str.Where(c => !d.Keys.Contains(c)))
            {
                d.Add(c, str.Count(ch => ch == c));
                it++;
            }
            var indice = d.Keys.Aggregate(0.0, (current, key) => current + ((d[key]*(d[key] - 1.0))/(str.Length*(str.Length - 1))));
            Console.WriteLine(indice.ToString("0.00000") + " iter : " + it);
            Console.ReadKey();
        }

        private static void Kasiski(string texte)
        {
            var St = new List<string>();
            for (var i = 2; i < 10; i++)
            {
                for (var j = 0; j < texte.Length; j+=i)
                {
                    var str = texte.Skip(j).Take(i).AsString();
                    if (texte.Count(str) <= 4) continue;
                    St.Add(str);
        ...
        public static int Bezout(int n)
        {
            return 0;
        }
    }
    ...
    {
        public static string AsString(this IEnumerable<char> c)
        {
            return c.Aggregate(string.Empty, (current, ch) => current + c);
        }

        public static int Count(this string str, string toSearch)
        {
            return str.Where((t, i) => str.Skip(i).Take(toSearch.Length).AsString() == toSearch).Count();
        }
